import { ColorType, createChart, IChartApi, SeriesMarker, SeriesMarkerPosition, Time } from "lightweight-charts";
...
export default ({ data }: IUseChartProps) => {
    ...
    useEffect(() => {
        if (chartContainerRef.current) {
            chartRef.current = createChart(chartContainerRef.current, {
                ...
                rightPriceScale: {  // 配置右侧价格轴（K线图）
                    visible: true,
                    autoScale: true,
                },
                leftPriceScale: {  // 配置左侧价格轴（权益曲线）
                    visible: true,
                    borderVisible: true,
                    autoScale: true,
            ...
            chartRef.current.subscribeCrosshairMove(param => {
                if (param.time && param.point) {
                    ...
                    const trade = data.trades.find(t =>
                        new Date(t.timestamp).getTime() / 1000 === param.time
                    );
                    const equity = data.equity.find(e =>
                        new Date(e.timestamp).getTime() / 1000 === param.time
                    );
                    ...
                    if (price) {
                        ...
                        // 使用不同颜色显示 OHLC
                        const closeColor = price.close >= price.open ? '#26a69a' : '#ef5350';
                        legend.innerHTML = `
                            <div style="color: #666">Open: ${price.open.toFixed(2)}</div>
                            <div style="color: #26a69a">High: ${price.high.toFixed(2)}</div>
                            <div style="color: #ef5350">Low: ${price.low.toFixed(2)}</div>
                            <div style="color: ${closeColor}">Close: ${price.close.toFixed(2)}</div>
                            ${equity ? `
                            ...
                            ${trade ? `
                                <div style="margin-top: 4px; padding-top: 4px; border-top: 1px solid #eee;">
                                    <div style="color: ${trade.action === 'BUY' ? '#26a69a' : '#ef5350'}">
